...
public sealed class CreateVehicleHandler(IMapper mapper, IVehicleRepository repository, IPersonRepository personRepository) : IRequestHandler<CreateVehicleCommand, Response<Vehicle>>
{
    public async Task<Response<Vehicle>> Handle(CreateVehicleCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return ResponseFactory.Fail<Vehicle>("Only clients are allowed to register a vehicle");
        }

        var mapperEntity = mapper.Map<Vehicle>(request);
        if (!mapperEntity.LicensePlate.IsValid())
        {
            return ResponseFactory.Fail<Vehicle>("Invalid license plate format");
        }
