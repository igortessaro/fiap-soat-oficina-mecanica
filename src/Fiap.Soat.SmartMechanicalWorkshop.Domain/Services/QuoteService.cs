...
public sealed class QuoteService(IMapper mapper, IQuoteRepository quoteRepository) : IQuoteService
{
    public async Task<Response<QuoteDto>> CreateAsync(ServiceOrderDto serviceOrder, CancellationToken cancellationToken)
    {
        ...
        {
            return ResponseFactory.Fail<QuoteDto>(new Error($"Service order must be in {nameof(ServiceOrderStatus.WaitingApproval)} status to create a quote"), System.Net.HttpStatusCode.BadRequest);
        }

        if (!serviceOrder.AvailableServices.Any())
        {
            return ResponseFactory.Fail<QuoteDto>(new FluentResults.Error("Service order does not have available services"), System.Net.HttpStatusCode.BadRequest);
        }
        ...
        return ResponseFactory.Ok<QuoteDto>(mapper.Map<QuoteDto>(entity));
    }
    ...
    public async Task<Response<QuoteDto>> PatchAsync(Guid id, QuoteStatus status, CancellationToken cancellationToken)
    {
        ...
        if (quote is null)
        {
            return ResponseFactory.Fail<QuoteDto>(new FluentResults.Error("Quote not found"), System.Net.HttpStatusCode.NotFound);
        }
        ...
        switch(status)
        {
            ...
            default:
                return ResponseFactory.Fail<QuoteDto>(new FluentResults.Error($"Invalid status {status}"), System.Net.HttpStatusCode.BadRequest);
        }
