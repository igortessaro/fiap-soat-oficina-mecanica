...
public sealed class ClientService(IMapper mapper, IClientRepository repository) : IClientService
{
    public async Task<Response<ClientDto>> CreateAsync(CreateClientRequest request, CancellationToken cancellationToken)
    {
        ...
        return ResponseFactory.Ok(mapper.Map<ClientDto>(createdEntity), System.Net.HttpStatusCode.Created);
    }
    ...
    public async Task<Response> DeleteAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        {
            return ResponseFactory.Fail(new FluentResults.Error("Client not found"), System.Net.HttpStatusCode.NotFound);
        }
    ...
    public async Task<Response<ClientDto>> GetOneAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        return foundEntity != null
            ? ResponseFactory.Ok(mapper.Map<ClientDto>(foundEntity))
            : ResponseFactory.Fail<ClientDto>(new FluentResults.Error("Client Not Found"), System.Net.HttpStatusCode.NotFound);
    }
    ...
    public async Task<Response<ClientDto>> UpdateAsync(UpdateOneClientInput input, CancellationToken cancellationToken)
    {
        ...
        if (foundEntity is null)
        {
            return ResponseFactory.Fail<ClientDto>(new FluentResults.Error("Client not found"), System.Net.HttpStatusCode.NotFound);
        }
        ...
        return ResponseFactory.Ok(mapper.Map<ClientDto>(updatedEntity));
    }
